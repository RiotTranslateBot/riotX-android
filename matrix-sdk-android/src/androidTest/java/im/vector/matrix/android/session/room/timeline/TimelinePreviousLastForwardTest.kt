...
class TimelinePreviousLastForwardTest : InstrumentedTest {
    ...
    fun previousLastForwardTest() {
        ...
        // Bob navigate to the first event (room creation event)
        run {
            val lock = CountDownLatch(1)
            val eventsListener = commonTestHelper.createEventListener(lock) { snapshot ->
                Timber.e("Bob timeline updated: with ${snapshot.size} events:")
        ...
        run {
            ...
            val eventsListener = commonTestHelper.createEventListener(lock) { snapshot ->
                ...
                snapshot.lastOrNull()?.root?.getClearType() == EventType.STATE_ROOM_CREATE
                        // 8 for room creation item, and 50 for the forward pagination
                        && snapshot.size == 8 + 25 + 50
            }

            bobTimeline.addListener(eventsListener)

            // FIXME It does not send any request here, because nextToken of the current chunk is null
        ...
        // Bob scroll again to the future, till the live
        run {
            val lock = CountDownLatch(1)
            val eventsListener = commonTestHelper.createEventListener(lock) { snapshot ->
                Timber.e("Bob timeline updated: with ${snapshot.size} events:")
